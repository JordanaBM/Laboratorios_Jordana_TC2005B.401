...
exports.post_nueva_peli = (request, response, next) => {    
    const peli = new Peliculas(request.body.nombre, request.body.sinopsis, request.file.filename);
    peli.save().then(() => {
...
exports.post_nueva_serie = (request, response, next) => {
    ...
    const serie = new Series(request.body.nombre, request.body.sinopsis, request.file.filename);
    serie.save().then(() => {
...
exports.post_nueva_caricatura = (request, response, next) => {
    ...
    const cari = new Caricaturas(request.body.nombre, request.body.sinopsis, request.file.filename);
    cari.save().then(() => {
...
exports.buscarPelicula = (request, response, next) => {
    console.log(request.params.valor);
    Peliculas.fetch(request.params.valor)
        .then(([rows, fieldData]) => {
            console.log(rows);
